...
body{font-family:'Fredoka One',cursive;overflow:hidden;height:100vh;width:100vw;max-width:100%;max-height:100%;
  background:linear-gradient(180deg,#87CEEB 0%,#B8E8FF 52%,#6EC840 52%,#4DA830 100%);position:relative;touch-action:manipulation;}
...
.choice-card svg{width:110%;height:110%;pointer-events:none;animation:objBounce 2.6s ease-in-out infinite;}
@keyframes objBounce{0%{transform:scale(1) rotateZ(0deg);}25%{transform:scale(1.12) rotateZ(-3deg);}50%{transform:scale(0.98) rotateZ(2deg);}75%{transform:scale(1.08) rotateZ(-2deg);}100%{transform:scale(1) rotateZ(0deg);}}
...
/* ══════════════════════════════════════════
   GAME STATE
══════════════════════════════════════════ */
let levels=[],cur=0,score=0,answered=false,streak=0;

function restart(){
  levels=buildLevels();cur=0;score=0;answered=false;streak=0;
  document.getElementById('win-screen').style.display='none';
  setTimeout(()=>waveMascot(),800);
  load();
}
...
function cheerMascot(){
  if(streak>=10)impressedMascot();
  else if(streak>=5)danceMascot();
  else basicCheer();
}
...
═══════════════════════════════════════════ */
const REACTIONS={
  cheer:['WOOOW!','Amazing!','Superstar!','Perfect!','Brilliant!'],
  dance:['Rock Star!','Incredible!','That was AWESOME!','You Crush It!','So Cool!'],
  thinking:['Hmm...','Think...','What could it be?'],
  hide:['Oops!','Oh no!','Try again!','No worries!'],
  nod:['Good try!','Getting close!','Almost there!','You got this!'],
  wave:['Hello!','Welcome!','Let\'s play!','Let\'s go!'],
  impressed:['WOW!','GENIUS!','Outstanding!','Unbelievable!','LEGEND!']
};
...
function init3DCat(){
  try{
    ...
    mascotScene=new THREE.Scene();mascotScene.background=new THREE.Color(0x000000);
